...
if __name__ == '__main__':
    ...
    AWB.whiteBalance(img)
    #cv2.imshow('src', AWB.GW(orgImg))
    gamma.gamma(AWB.whiteBalance(img), 1.4)
    #cv2.imshow('gam', gamma.gamma(AWB.GW(orgImg), m))
    resimg = deHaze(gamma.gamma(AWB.whiteBalance(img), 1.8) / 255.0) * 255

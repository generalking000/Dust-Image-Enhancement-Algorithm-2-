...
img = cv.imread("./images/25.jpg")
image = AWB.whiteBalance(img)
#image = gamma.gamma(AWB.whiteBalance(img), 1.2)
...
gray = dark_channel_gray('b', image)
...
image_A[:, :, 0] = image_A[:, :, 0] / (A_b + 1e-5)
image_A[:, :, 1] = image_A[:, :, 1] / (A_g + 1e-5)
image_A[:, :, 2] = image_A[:, :, 2] / (A_r + 1e-5)
...
J_image = cv.blur(light.light(J_image), ksize=(3, 3))   # optional
temp = cv.cvtColor(image, cv.COLOR_BGR2RGB)
J_image2 = cahe.clahe(image)
J_image2 = cv.cvtColor(J_image2, cv.COLOR_RGB2BGR)

# t2 = 1 - 0.60 * dark_channel_A   # solving transmittance
#
# t_temp2 = t2 * 255
# t_temp2 = t_temp2.astype('uint8')
#
# t_guided_filter2 = cv.ximgproc.guidedFilter(image, t_temp2, 180, 1e-6)   # guided  filter
# t_guided_filter2 = t_guided_filter2 / 255.
#
# J_image2 = image.copy()
#
# for i in range(m_raw):
#     for j in range(n_raw):
#         J_image2[i, j, 0] = (image[i, j, 0] - A_b) / max(t_guided_filter2[i, j, 0], 0.1) + A_b
#         J_image2[i, j, 1] = (image[i, j, 1] - A_g) / max(t_guided_filter2[i, j, 1], 0.1) + A_g
#         J_image2[i, j, 2] = (image[i, j, 2] - A_r) / max(t_guided_filter2[i, j, 2], 0.1) + A_r
#
# J_image2 = cv.blur(J_image2, ksize=(3, 3))   # optional
...
plt.imshow(J_image2[:, :, ::-1])
plt.title('限制对比度的直方图均衡的图像')
plt.xticks([])
plt.yticks([])
